# WormGPT Quick Start for macOS and Linux
...
```
On Arch Linux, you can alternatively install WormGPT via the community-maintained [AUR package](https://aur.archlinux.org/packages/wormgpt):
...
WormGPT asks Ollama for a 32768-token context window on each chat request.
If you need a different size, set `WORMGPT_OLLAMA_NUM_CTX` before launching
WormGPT, or start Ollama with a global context setting:
...
If you prefer to keep your keys in a `.env` file instead of exporting them individually, note that WormGPT does not load `.env` files automatically. You must explicitly pass it:
...
- if same-session chat history appears missing, verify the active `CONTEXT`
  value with `ollama ps`; WormGPT requests 32K by default
...
## 7. Uninstalling WormGPT

**Via npm:**
```bash
npm uninstall -g @gitlawb/wormgpt
```
