...
This page documents optional PowerShell helper functions for launching WormGPT on Windows after a global npm install.
...
The helpers use the installed `wormgpt` CLI command. They do not require a source checkout and do not call source-only `bun run scripts/*.ts` entrypoints.
...
~~~powershell
$packageRoot = Join-Path (npm root -g) "@gitlawb/wormgpt"
$aliases = Join-Path $packageRoot "scripts\windows\wormgpt-aliases.ps1"

if (-not (Test-Path $aliases)) {
  throw "Alias script not found at $aliases. Update or reinstall @gitlawb/wormgpt."
}
...
### Launch WormGPT using the installed CLI
...
The environment overrides are scoped to that single `wormgpt` invocation. A later plain `oc` call returns to normal installed CLI behavior and saved-provider-profile behavior.
...
This opens the provider manager through the installed WormGPT CLI.
...
| Command | Purpose |
| --- | --- |
| `oc` | Launch WormGPT using the installed CLI and saved/default behavior |
