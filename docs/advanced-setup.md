...
```bash
git clone https://github.com/Gitlawb/wormgpt.git
cd wormgpt
...
If you use the in-app provider wizard, choose `Codex OAuth` to open ChatGPT sign-in in your browser and let WormGPT store Codex credentials securely.

If you already use the Codex CLI, WormGPT reads `~/.codex/auth.json` automatically. You can also point it elsewhere with `CODEX_AUTH_JSON_PATH` or override the token directly with `CODEX_API_KEY`.
...
wormgpt --model claude-sonnet-4-6
```
...
WormGPT sends the current conversation history to Ollama on each turn and
uses Ollama's native chat API for Ollama endpoints. Native chat lets WormGPT
...
`WORMGPT_OLLAMA_NUM_CTX` before launching WormGPT:

```bash
export WORMGPT_OLLAMA_NUM_CTX=65536
```
...
new WormGPT request, stop the existing Ollama app/server, start it again, and
retry the request.
...
The **WormGPT VS Code extension** can store the key in Secret Storage and set these variables for you when you launch from the Control Center. See `vscode-extension/wormgpt-vscode/README.md`.
...
| `WORMGPT_OLLAMA_NUM_CTX` | Ollama only | Request-level Ollama context window. Defaults to `32768`; set a larger value for longer same-session history if your model and hardware can handle it. |
...
| `WORMGPT_MAX_RETRIES` | No | Maximum retry attempts for retryable API failures, capped at 100 (default: 10). Set to `0` to disable retries after the initial request. If unset, deprecated `CLAUDE_CODE_MAX_RETRIES` is still honored for compatibility. |
| `WORMGPT_RETRY_DELAY_MS` | No | Base retry delay in milliseconds for APIs that do not send `Retry-After`; exponential backoff starts from this value, capped at 60000 (default: 500) |
| `WORMGPT_QUERY_HARD_MAX_MS` | No | Foreground query hard maximum in milliseconds. Defaults to 1800000 (30 minutes). Use a larger positive integer for long autonomous sessions; invalid, zero, negative, fractional, or timer-overflow values are ignored with a warning. |
| `WORMGPT_DISABLE_CO_AUTHORED_BY` | No | Suppress the default `Co-Authored-By` trailer in generated git commits |
| `WORMGPT_LOG_TOKEN_USAGE` | No | When truthy (e.g. `verbose`), emits one JSON line on stderr per API request with input/output/cache tokens and the resolved provider. **User-facing debug output** — complements the REPL display controlled by `/config showCacheStats`. Distinct from `CLAUDE_CODE_ENABLE_TOKEN_USAGE_ATTACHMENT`, which is **model-facing** (injects context usage info into the prompt itself). Both can run together. |
...
wormgpt doctor report --markdown

# write a redacted JSON issue report for attachment
wormgpt doctor report --json --out wormgpt-report.json

# write a deterministic task report from a session transcript
wormgpt report --json --transcript ~/.wormgpt/projects/-path-to-project/session-id.jsonl --out task-report.json

# print a human-readable task report from the latest session in the current project
wormgpt report --markdown
...
- `wormgpt doctor report` is redacted by default and is intended for GitHub issues. It summarizes provider/runtime/build/settings state without prompts, transcripts, raw settings files, API keys, MCP command details, or full home-directory paths.
- `wormgpt report --json` and `wormgpt report --markdown` summarize observed session facts such as tool uses, Bash commands, validation commands, changed files, branch metadata, warnings, and linked issue/PR references. Use `--transcript <file>` for an explicit transcript, `--session <id>` for a stored session, or omit both to report the latest session for the current project. Large previews are truncated and credential-shaped strings are redacted. When no validation command is observed, the report keeps `validations` empty and includes a warning instead of claiming checks passed.
...
Configure this in `~/.wormgpt.json`:
...
By default, WormGPT compacts conversations based on token usage and also
...
would otherwise prevent it. Set `WORMGPT_MAX_ACTIVE_MESSAGES_HARD_CAP=0`
...
The legacy `WORMGPT_MAX_ACTIVE_MESSAGES` environment variable is still
honored when the setting is `off`. `WORMGPT_MAX_ACTIVE_MESSAGES_HARD_CAP`
