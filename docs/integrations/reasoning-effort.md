...
WormGPT treats reasoning support as a per-model capability. Provider and gateway catalogs can contain a mix of reasoning and non-reasoning models, so reasoning controls must never be inferred provider-wide.

## Concepts

`capabilities.supportsReasoning` means the model is known to support reasoning or thinking behavior. It is safe capability metadata, but by itself it does not authorize WormGPT to mutate API requests.

`reasoning` describes the request control surface WormGPT can safely use for that exact model entry or model descriptor.
...
1. Probe the exact route and model ID WormGPT will send.
