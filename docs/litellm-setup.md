...
WormGPT can connect to LiteLLM through LiteLLM's OpenAI-compatible proxy.

## Overview

LiteLLM is an open-source LLM gateway that provides a unified API to 100+ model providers. By running the LiteLLM Proxy, you can route WormGPT requests through LiteLLM to access any of its supported providers — all while using WormGPT's existing OpenAI-compatible provider path.
...
- LiteLLM's OpenAI-compatible endpoint accepts the same request format as OpenAI, so WormGPT works without custom request shaping.
- WormGPT discovers LiteLLM model context from `/v1/models` when LiteLLM exposes `context_length`, `context_window`, `max_model_len`, or `max_input_tokens`, including under `model_info`.
- You can switch between any provider configured in LiteLLM by simply changing the `OPENAI_MODEL` value — no need to reconfigure WormGPT.

### Context window detection

For custom LiteLLM aliases, add context metadata to each model entry when the
upstream model supports a larger window than WormGPT's fallback:
...
| `/context` shows 128K for a larger model | LiteLLM is not exposing context metadata for the alias, or startup discovery has not refreshed | Add `model_info.context_length` or `model_info.max_input_tokens` to the LiteLLM config, restart the proxy, then restart WormGPT; use `CLAUDE_CODE_OPENAI_CONTEXT_WINDOWS` as an explicit override if needed |
