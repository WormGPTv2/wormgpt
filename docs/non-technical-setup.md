# WormGPT for Non-Technical Users
...
WormGPT lets you use an AI coding assistant with different model providers such as:
...
Once you have picked a provider, run `/provider` inside WormGPT to set it up with guided prompts.
...
After you run `wormgpt`, the CLI should start and wait for your prompt.
...
2. Run `/provider` inside WormGPT to update your credentials
3. Re-run `wormgpt`
...
- WormGPT does not automatically load `.env` files. If you copied `.env.example` to `.env`, WormGPT won't see the variables unless you tell it to.

Fix:

- Load the file explicitly:
  `wormgpt --provider-env-file .env`
- Or, use the `/provider` command inside WormGPT instead (recommended).
...
- **GitHub Discussions**: https://github.com/Gitlawb/wormgpt/discussions
  Use this for Q&A, setup help, and community conversation.

- **GitHub Issues**: https://github.com/Gitlawb/wormgpt/issues
  Use this for confirmed bugs and feature requests.

### Quick diagnostic check

If WormGPT is not working after setup, run:
