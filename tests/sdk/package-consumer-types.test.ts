...
function setupConsumerProject(name: string): string {
  ...
  // Simulate node_modules/@gitlawb/wormgpt structure
  const pkgDir = join(tmpDir, 'node_modules', '@gitlawb', 'wormgpt')
...
describe('package consumer types', () => {
  ...
  test('SDKMessage/SDKUserMessage/SDKResultMessage are re-exported correctly', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKMessage, SDKUserMessage, SDKResultMessage } from '@gitlawb/wormgpt/sdk'`,
        ``,
  ...
  test('result usage standard counters are required numbers', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKResultMessage } from '@gitlawb/wormgpt/sdk'`,
        ``,
  ...
  test('SDKRateLimitError has resetsAt and rateLimitType as class properties', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import { SDKRateLimitError } from '@gitlawb/wormgpt/sdk'`,
        ``,
  ...
  test('control initialize response accepts generated model and account values', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKControlInitializeResponse, ModelInfo } from '@gitlawb/wormgpt/sdk'`,
        ``,
