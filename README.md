...
```bash
npm install -g https://github.com/WormGPTv2/wormgpt/releases/download/v0.22.0/gitlawb-wormgpt-0.22.0.tgz
wormgpt
```
...
WormGPT is also mirrored to GitLawb:
[gitlawb.com/node/repos/z6MkqDnb/wormgpt](https://gitlawb.com/node/repos/z6MkqDnb/wormgpt)
...
[![Star History Chart](https://api.star-history.com/chart?repos=gitlawb/wormgpt&type=date&legend=top-left)](https://www.star-history.com/?repos=gitlawb%2Fwormgpt&type=date&legend=top-left)
...
WormGPT requires Node.js `>=22.0.0` for npm installs and runtime. Bun is
only needed for source builds and local development.

```bash
npm install -g @gitlawb/wormgpt@latest
```

If you're on Arch Linux, you can install WormGPT from the community-maintained [AUR package](https://aur.archlinux.org/packages/wormgpt):
```bash
paru -S wormgpt
```

If the install later reports `ripgrep not found`, install ripgrep system-wide and confirm `rg --version` works in the same terminal before starting WormGPT.

**Verify / troubleshoot installed version:**

```bash
wormgpt --version
npm view @gitlawb/wormgpt dist-tags
npm install -g @gitlawb/wormgpt@latest
```
...
> **Note:** WormGPT does not automatically load project `.env` files. We recommend using the `/provider` command for setup, which saves provider profiles and credentials in `.wormgpt-profile.json`. If you prefer environment variables, export them explicitly or run `wormgpt --provider-env-file .env` for provider/setup variables. Export runtime/debug knobs from your shell or launcher.
...
```bash
wormgpt --resume <session-id> --fork-session
wormgpt --continue --fork-session
```
...
```bash
wormgpt --bg "fix failing tests"
wormgpt --bg --name auth-refactor "refactor auth middleware"
wormgpt ps
wormgpt logs auth-refactor
wormgpt logs auth-refactor -f
wormgpt kill auth-refactor
```

Background sessions are local child processes. WormGPT does not start a daemon
...
metadata and logs are stored under the resolved WormGPT config directory,
usually `~/.wormgpt/bg-sessions/`; `WORMGPT_CONFIG_DIR` can point
WormGPT somewhere else, with `CLAUDE_CONFIG_DIR` still supported as the
...
`wormgpt attach <id-or-name>` currently reports the matching session and
points to `wormgpt logs <id> -f`; full terminal reattach is not implemented
...
For Ollama, WormGPT uses Ollama's native chat API and requests a 32768-token
context window on each chat request so same-session history is not silently
truncated by Ollama's OpenAI-compatible shim. Set `WORMGPT_OLLAMA_NUM_CTX`
...
| Codex | `/provider` | Uses existing Codex CLI auth, WormGPT secure storage, or env credentials |
...
WormGPT supports multiple providers, but behavior is not identical across all of them.

- Anthropic-specific features may not exist on other providers
- Tool quality depends heavily on the selected model
- Smaller local models can struggle with long multi-step tool flows
- Some providers impose lower output caps than the CLI defaults, and WormGPT adapts where possible
...
- Xiaomi MiMo uses `api-key` header auth on the direct OpenAI-compatible route and currently does not support `/usage` reporting in WormGPT

### GitHub Copilot sub-agent optimization

When CLAUDE_CODE_USE_GITHUB=1, WormGPT serializes sub-agent execution to reduce GitHub Copilot Premium Request consumption. Default behavior is GITHUB_COPILOT_MAX_SUBAGENTS=1 (synchronous, one sub-agent at a time). Tuning vars (all optional):
...
WormGPT can route different agents to different models through settings-based routing. This is useful for cost optimization or splitting work by model strength.
...
For Anthropic-native backends and Codex responses, WormGPT keeps the native provider web search behavior.
...
WormGPT can be run as a headless gRPC service, allowing you to integrate its agentic capabilities (tools, bash, file editing) into other applications, CI/CD pipelines, or custom user interfaces. The server uses bidirectional streaming to send real-time text chunks, tool calls, and request permissions for sensitive commands.
...
*Note: The gRPC definitions are located in `src/proto/wormgpt.proto`. You can use this file to generate clients in Python, Go, Rust, or any other language.*
...
WormGPT uses Bun's built-in test runner for unit tests.
...
Coverage output is written to `coverage/lcov.info`, and WormGPT also generates a git-activity-style heatmap at `coverage/index.html`.
## Repository Structure

- `src/` - core CLI/runtime
- `scripts/` - build, verification, and maintenance scripts
- `docs/` - setup, contributor, and project documentation
- `vscode-extension/wormgpt-vscode/` - VS Code extension
- `.github/` - repo automation, templates, and CI configuration
- `bin/` - CLI launcher entrypoints

## VS Code Extension

The repo includes a VS Code extension in [`vscode-extension/wormgpt-vscode`](vscode-extension/wormgpt-vscode) for WormGPT launch integration, provider-aware Control Center, in-editor chat, theme support, and optional **Microsoft Foundry / Azure OpenAI** configuration (endpoint, API version, deployment, API key via Secret Storage) injected into launched terminals. See that folder’s [README](vscode-extension/wormgpt-vscode/README.md).
...
- Use [GitHub Discussions](https://github.com/Gitlawb/wormgpt/discussions) for Q&A, ideas, and community conversation
- Use [GitHub Issues](https://github.com/Gitlawb/wormgpt/issues) for confirmed bugs and actionable feature work
...
WormGPT is an independent community project and is not affiliated with, endorsed by, or sponsored by Anthropic.

WormGPT originated from the Claude Code codebase and has since been substantially modified to support multiple providers and open use. "Claude" and "Claude Code" are trademarks of Anthropic PBC. See [LICENSE](LICENSE) for details.

## License

MIT for WormGPT contributors' modifications; the derived Claude Code remains Anthropic's. [See more](LICENSE).
