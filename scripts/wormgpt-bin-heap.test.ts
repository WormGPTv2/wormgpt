...
const BIN_PATH = join(import.meta.dir, '..', 'bin', 'wormgpt')

describe('wormgpt launcher heap guard', () => {
  ...
  test('keeps user and troubleshooting escape hatches', () => {
    ...
    expect(source).toContain('WORMGPT_DISABLE_HEAP_RELAUNCH')
    expect(source).toContain('WORMGPT_NODE_MAX_OLD_SPACE_SIZE_MB')
