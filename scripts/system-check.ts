...
export function buildMemoryGuardChecks(
  ...
  const hardCapOverride = env.WORMGPT_MAX_ACTIVE_MESSAGES_HARD_CAP
  ...
  const configuredLimit =
    ...
    input.maxMessagesCompactionThreshold !== 'off'
      ? input.maxMessagesCompactionThreshold
      : undefined
  const legacyLimit = parsePositiveInteger(env.WORMGPT_MAX_ACTIVE_MESSAGES)
  const memoryBudget = parsePositiveInteger(env.WORMGPT_MAX_MEMORY_MB) || 1536
  ...
  results.push(
    hardCap === 0
      ? fail(
          ...
          'Disabled by WORMGPT_MAX_ACTIVE_MESSAGES_HARD_CAP=0; long sessions can grow without the active-message safety cap.',
        )
      : pass(
          'Active-message hard cap',
...
export function readNodeExecutableVersion(
  ...
  if (result.error) {
    return {
      ok: false,
      detail: `Unable to run \`node --version\`: ${result.error.message}. WormGPT requires Node.js ${MIN_NODE_ENGINE_RANGE} on PATH.`,
    }
  }
  ...
  if (result.status !== 0) {
    ...
    return {
      ok: false,
      detail: `Unable to run \`node --version\`${suffix}. WormGPT requires Node.js ${MIN_NODE_ENGINE_RANGE} on PATH.`,
    }
  }
  ...
  if (!version) {
    return {
      ok: false,
      detail: `Unable to read Node.js version from \`node --version\`. WormGPT requires Node.js ${MIN_NODE_ENGINE_RANGE} on PATH.`,
    }
  }
