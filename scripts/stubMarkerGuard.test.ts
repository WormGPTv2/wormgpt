...
test('canonicalStub keys on the src-relative path across separators', () => {
  expect(canonicalStub('/home/runner/work/wormgpt/wormgpt/src/commands/dream/dream.ts')).toBe(
    'src/commands/dream/dream',
  )
  // Windows separators normalize to the same key.
  expect(canonicalStub('C:\\repo\\wormgpt\\src\\commands\\dream\\dream.ts')).toBe(
    'src/commands/dream/dream',
  )
})

test('collectBundleStubs parses the JSON string-literal marker (minified builds)', () => {
  const bundle = `;(globalThis.__wormgptStubMarkers ??= []).push("missing-module-stub:/build/src/utils/foo.js");`
...
test('collectBundleStubs keeps Windows paths intact in the string-literal marker', () => {
  ...
  const marker = JSON.stringify('missing-module-stub:C:\\repo\\wormgpt\\src\\commands\\dream\\dream.js')
  const bundle = `;(globalThis.__wormgptStubMarkers ??= []).push(${marker});`
...
test('collectBundleStubs keeps spaced Windows paths intact in the string-literal marker', () => {
  const marker = JSON.stringify(
    'missing-module-stub:C:\\Users\\Jane Doe\\wormgpt\\src\\commands\\dream\\dream.js',
  )
  const bundle = `;(globalThis.__wormgptStubMarkers ??= []).push(${marker});`
...
test('collectBundleStubs keeps spaced paths intact in the comment marker', () => {
  const bundle = `// missing-module-stub:/home/jane doe/wormgpt/src/utils/foo.js\nvar foo = {};`
...
test('collectBundleStubs handles paths containing spaces', () => {
  const marker = JSON.stringify(
    'missing-module-stub:/Users/John Doe/projects/wormgpt/src/utils/foo.js',
  )
  const bundle = `;(globalThis.__wormgptStubMarkers ??= []).push(${marker});`
