/**
 * WormGPT build script — bundles the TypeScript source into a single
...
result = await Bun.build({
  ...
  define: {
    ...
    'MACRO.ISSUES_EXPLAINER':
      JSON.stringify('report the issue at https://github.com/Gitlawb/wormgpt/issues'),
    'MACRO.FEEDBACK_CHANNEL':
      JSON.stringify('https://github.com/Gitlawb/wormgpt/issues'),
    'MACRO.PACKAGE_URL': JSON.stringify('@gitlawb/wormgpt'),
        ...
        build.onLoad(
          ...
          (args) => {
            ...
            return {
              contents: `
const noop = () => null;
;(globalThis.__wormgptStubMarkers ??= []).push(${marker});
export default noop;
${exports}
`,
              loader: 'js',
...
if (!result.success) {
  ...
  console.log(`✓ Built wormgpt v${version} → dist/cli.mjs`)
}
...
if (result?.success) {
  ...
  // differs only by the repo-root prefix (`/home/ubuntu/.../wormgpt` locally
  // vs `/home/runner/work/wormgpt/wormgpt` on CI). canonicalStub() keys
