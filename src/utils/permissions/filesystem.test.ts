...
describe('auto-memory write permissions', () => {
  ...
  beforeEach(async () => {
    ...
    projectDir = await mkdtemp(join(tmpdir(), 'wormgpt-memory-perms-'))
  ...
  test('requires approval for overridden auto-memory writes', async () => {
    const overrideDir = await mkdtemp(join(tmpdir(), 'wormgpt-memory-'))
...
describe('WormGPT commit message temp file permissions', () => {
  let originalCwd: string
  let projectDir: string

  beforeEach(async () => {
    originalCwd = getOriginalCwd()
    projectDir = await mkdtemp(join(tmpdir(), 'wormgpt-perms-'))
  ...
  test('allows the project-local WORMGPT_COMMIT_MSG file without a safety prompt', () => {
    const result = checkWritePermissionForTool(
      writeTool,
      { file_path: join(projectDir, '.git', 'WORMGPT_COMMIT_MSG') },
      permissionContext('bypassPermissions'),
    )
  ...
  test('allows the project-local WORMGPT_COMMIT_MSG file in fullAccess mode', () => {
    const result = checkWritePermissionForTool(
      writeTool,
      { file_path: join(projectDir, '.git', 'WORMGPT_COMMIT_MSG') },
  ...
  test('does not allow same-named files outside the project git directory', () => {
    ...
    const result = checkWritePermissionForTool(
      writeTool,
      { file_path: join(otherDir, '.git', 'WORMGPT_COMMIT_MSG') },
      permissionContext('bypassPermissions'),
    )
