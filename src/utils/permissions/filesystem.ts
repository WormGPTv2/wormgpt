 ...
 * .wormgpt/skills/{name}/ directory (global), plus the legacy global
...
export function getClaudeSkillScope(
  ...
  const bases = [
    ...
    {
      dir: expandPath(join(homedir(), '.wormgpt', 'skills')),
      prefix: '~/.wormgpt/skills/',
    },
    {
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  if (
    normalizedPath.endsWith(`${sep}.wormgpt${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.wormgpt${sep}settings.local.json`) ||
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  const openCommandsDir = join(getOriginalCwd(), '.wormgpt', 'commands')
  const openAgentsDir = join(getOriginalCwd(), '.wormgpt', 'agents')
  const openSkillsDir = join(getOriginalCwd(), '.wormgpt', 'skills')
...
export function isWormGPTCommitMessagePath(absolutePath: string): boolean {
  const expectedPath = join(getOriginalCwd(), '.git', 'WORMGPT_COMMIT_MSG')
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (claudeFolderAllowRule) {
    ...
    // '~/.wormgpt/**', and legacy '~/.claude/**') plus narrowed skill
    // patterns like '~/.wormgpt/skills/my-skill/**' so users can grant
...
export function checkEditableInternalPath(
  ...
  if (
    ...
    isWormGPTCommitMessagePath(normalizedPath)
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
      decisionReason: {
        type: 'other',
        reason: 'WormGPT commit message file is allowed for writing',
      },
    }
  }
