...
function wormgptEntryNames(type: 'mark' | 'measure', scope: string): string[] {
  const names = markNames(type)
  const namespacedPrefix = `wormgpt:${scope}:`
  if (scope === 'query') {
    return names.filter(
...
function handlePromptSubmitProfilerScript(
  ...
  return `
    ...
    console.log(JSON.stringify({
      onQueryCalls,
      queryMarks: marks.filter(name => name.startsWith('wormgpt:query:')),
      queryMeasures: measures.filter(name => name.startsWith('wormgpt:query:')),
...
beforeEach(async () => {
  ...
  tempConfigDir = mkdtempSync(join(tmpdir(), 'wormgpt-profiler-retention-'))
  ...
  for (const scope of ['query', 'headless', 'startup']) {
    for (const type of ['mark', 'measure'] as const) {
      clearEntriesByName(wormgptEntryNames(type, scope))
    }
  }
})
...
test('query profile report is logged before scoped cleanup and unrelated entries survive', async () => {
  ...
  expect(wormgptEntryNames('mark', 'query')).toEqual([])
  expect(wormgptEntryNames('measure', 'query')).toEqual([])
...
test('handle prompt submit preserves query profile when onQuery owns cleanup', () => {
  ...
  expect(result.queryMarks).toEqual(
    expect.arrayContaining([
      'wormgpt:query:query_user_input_received',
      'wormgpt:query:query_process_user_input_start',
      'wormgpt:query:query_process_user_input_end',
    ]),
  )
  expect(result.queryMeasures).toEqual([])
  expect(result.externalMarkRetained).toBe(true)
...
test('headless turn logging extracts metrics and clears retained marks', async () => {
  ...
  expect(wormgptEntryNames('mark', 'headless')).toEqual([])
  expect(wormgptEntryNames('measure', 'headless')).toEqual([])
  expect(markNames('mark')).toContain('external_profiler_retention_start')
})
...
test('startup profile report keeps report names but clears one-shot entries', async () => {
  ...
  expect(wormgptEntryNames('mark', 'startup')).toEqual([])
  expect(wormgptEntryNames('measure', 'startup')).toEqual([])
  expect(markNames('mark')).toContain('external_profiler_retention_start')
})
