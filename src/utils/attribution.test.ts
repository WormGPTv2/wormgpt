...
const originalEnv = {
  ...
  WORMGPT_DISABLE_CO_AUTHORED_BY:
    process.env.WORMGPT_DISABLE_CO_AUTHORED_BY,
...
const defaultPrAttribution =
  '🤖 Generated with [WormGPT](https://github.com/Gitlawb/wormgpt)'
...
beforeEach(async () => {
  ...
  delete process.env.WORMGPT_DISABLE_CO_AUTHORED_BY
...
describe('getDefaultCommitCoAuthorName', () => {
  ...
  it('uses the WormGPT email for commit attribution across providers', () => {
...
describe('getAttributionTexts', () => {
  ...
  it('preserves includeCoAuthoredBy true as an explicit old-default opt-in', () => {
    ...
    expect(attribution.commit).toEndWith(' <wormgpt@gitlawb.com>')
  ...
  it('uses WORMGPT_DISABLE_CO_AUTHORED_BY to disable the old default co-author trailer', () => {
    process.env.WORMGPT_DISABLE_CO_AUTHORED_BY = '1'
    useSettings({ includeCoAuthoredBy: true })
  ...
  it('does not let WORMGPT_DISABLE_CO_AUTHORED_BY override explicit commit attribution', () => {
    process.env.WORMGPT_DISABLE_CO_AUTHORED_BY = '1'
    useSettings({
