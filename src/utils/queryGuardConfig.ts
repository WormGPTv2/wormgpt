export const WORMGPT_QUERY_HARD_MAX_MS_ENV =
  'WORMGPT_QUERY_HARD_MAX_MS'
...
function warnInvalidQueryHardMax(
  ...
  log(
    `${WORMGPT_QUERY_HARD_MAX_MS_ENV} invalid value "${value}" (${reason}); using default query hard max`,
    { level: 'warn' },
  )
}
...
export function getQueryGuardOptionsFromEnv(
  ...
  const raw = env[WORMGPT_QUERY_HARD_MAX_MS_ENV]
