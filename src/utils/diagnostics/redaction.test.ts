...
describe('diagnostic redaction', () => {
  ...
  test('redacts secret-looking values even under harmless field names', () => {
    ...
    const redacted = redactDiagnosticObject({
      ...
      path: `${home}/private/wormgpt/src/file.ts`,
    ...
    expect(redacted.path).toBe('~/private/wormgpt/src/file.ts')
  ...
  test('redacts Windows-style home paths without matching sibling directories', () => {
    ...
    expect(
      redactHomePath(
        'debug path C:\\Users\\Alice\\AppData\\Roaming\\wormgpt',
        home,
      ),
    ).toBe('debug path ~\\AppData\\Roaming\\wormgpt')
    expect(redactHomePath('C:\\Users\\AliceOther\\wormgpt', home)).toBe(
      'C:\\Users\\AliceOther\\wormgpt',
    )
  })
