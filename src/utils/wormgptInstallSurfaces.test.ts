...
beforeEach(async () => {
  await acquireSharedMutationLock('utils/wormgptInstallSurfaces.test.ts')
})
...
test('install command displays ~/.local/bin/wormgpt on non-Windows', async () => {
  await mockEnvPlatform('darwin')

  const { getInstallationPath } = await importFreshInstallCommand()

  expect(getInstallationPath()).toBe('~/.local/bin/wormgpt')
})

test('install command displays wormgpt.exe path on Windows', async () => {
  await mockEnvPlatform('win32')

  const { getInstallationPath } = await importFreshInstallCommand()

  expect(getInstallationPath()).toBe(
    join(homedir(), '.local', 'bin', 'wormgpt.exe').replace(/\//g, '\\'),
  )
})

test('native installer uses wormgpt launcher for WormGPT package', async () => {
  ;(globalThis as Record<string, unknown>).MACRO = {
    PACKAGE_URL: '@gitlawb/wormgpt',
  }

  const { getBinaryName, getExecutableName } = await importFreshInstaller()

  expect(getBinaryName('linux-x64')).toBe('claude')
  expect(getExecutableName('linux-x64')).toBe('wormgpt')
  expect(getExecutableName('win32-x64')).toBe('wormgpt.exe')
})
...
test('deep-link protocol resolver uses wormgpt launcher for WormGPT package', async () => {
  ;(globalThis as Record<string, unknown>).MACRO = {
    PACKAGE_URL: '@gitlawb/wormgpt',
  }
  ...
  expect(getProtocolBinaryName('linux')).toBe('wormgpt')
  expect(getProtocolBinaryName('win32')).toBe('wormgpt.exe')
})
    ...
    void render(
      createElement(Install, {
        ...
        onDone: (result: string) => {
          try {
            expect(result).toBe('WormGPT installation completed successfully')
            resolve()
          } catch (error) {
            reject(error)
          }
...
test('cleanupNpmInstallations removes both wormgpt and legacy claude local install dirs', async () => {
  ...
  mock.module('./envUtils.js', () => ({
    ...realEnvUtils,
    getClaudeConfigHomeDir: () => join(homedir(), '.wormgpt'),
  }))

  const { cleanupNpmInstallations } = await importFreshInstaller()
  await cleanupNpmInstallations()

  expect(removedPaths).toContain(join(homedir(), '.wormgpt', 'local'))
  expect(removedPaths).toContain(join(homedir(), '.claude', 'local'))
})

test('cleanupNpmInstallations manual fallback removes wormgpt npm shim', async () => {
  await mockEnvPlatform('darwin')

  const testHome = join(process.cwd(), 'work', 'wormgpt-install-home-test')
  const npmPrefix = join(testHome, '.npm-global')
  const shimPath = join(npmPrefix, 'bin', 'wormgpt')
  ;(globalThis as Record<string, unknown>).MACRO = {
    PACKAGE_URL: '@gitlawb/wormgpt',
  }
  process.env.HOME = testHome
  process.env.USERPROFILE = testHome
  process.env.CLAUDE_CONFIG_DIR = join(testHome, '.wormgpt')
