...
describe('convertToSandboxRuntimeConfig', () => {
  ...
  beforeEach(async () => {
    ...
    tempRoot = await mkdtemp(join(tmpdir(), 'wormgpt-sandbox-adapter-'))
  ...
  test('denies canonical WormGPT settings files in changed cwd', () => {
    const config = convertToSandboxRuntimeConfig({} as SettingsJson)

    expect(config.filesystem.denyWrite).toContain(
      resolve(activeCwd, '.wormgpt', 'settings.json'),
    )
    expect(config.filesystem.denyWrite).toContain(
      resolve(activeCwd, '.wormgpt', 'settings.local.json'),
    )
    expect(config.filesystem.denyWrite).toContain(
