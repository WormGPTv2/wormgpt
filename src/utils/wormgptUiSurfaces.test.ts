...
const originalWormGPTConfigDir = process.env.WORMGPT_CONFIG_DIR

beforeEach(async () => {
  await acquireSharedMutationLock('wormgptUiSurfaces.test.ts')
  mock.restore()
...
afterEach(() => {
  try {
    ...
    if (originalWormGPTConfigDir === undefined) {
      delete process.env.WORMGPT_CONFIG_DIR
    } else {
      process.env.WORMGPT_CONFIG_DIR = originalWormGPTConfigDir
    }
  } finally {
    releaseSharedMutationLock()
  }
...
describe('WormGPT settings path surfaces', () => {
  test('isClaudeSettingsPath recognizes project .wormgpt settings files', () => {
    expect(
      isClaudeSettingsPath(
        join(process.cwd(), '.wormgpt', 'settings.json'),
      ),
    ).toBe(true)

    expect(
      isClaudeSettingsPath(
        join(process.cwd(), '.wormgpt', 'settings.local.json'),
      ),
    ).toBe(true)
  })

  test('permission save destinations point user settings to configured WORMGPT_CONFIG_DIR', async () => {
    const customConfigDir = join(homedir(), 'custom-wormgpt')
    process.env.WORMGPT_CONFIG_DIR = customConfigDir
    delete process.env.CLAUDE_CONFIG_DIR
  ...
  test('skills help surfaces point user skills to configured WORMGPT_CONFIG_DIR', async () => {
    const customConfigDir = join(homedir(), 'custom-wormgpt')
    process.env.WORMGPT_CONFIG_DIR = customConfigDir
    delete process.env.CLAUDE_CONFIG_DIR
  ...
  test('permission save destinations point project settings to .wormgpt', async () => {
    const { optionForPermissionSaveDestination } = await import(
      '../components/permissions/rules/AddPermissionRules.tsx'
    )
    ...
    expect(optionForPermissionSaveDestination('projectSettings')).toEqual({
      ...
      description: 'Checked in at .wormgpt/settings.json',
      value: 'projectSettings',
    })

    expect(optionForPermissionSaveDestination('localSettings')).toEqual({
      label: 'Project settings (local)',
      description: 'Saved in .wormgpt/settings.local.json',
      value: 'localSettings',
    })
  })

  test('permission dialog treats ~/.wormgpt as the global Claude folder', () => {
    process.env.CLAUDE_CONFIG_DIR = join(homedir(), '.wormgpt')

    expect(
      isInGlobalClaudeFolder(
        join(homedir(), '.wormgpt', 'settings.json'),
      ),
    ).toBe(true)
    expect(
  ...
  test('permission dialog does not treat arbitrary CLAUDE_CONFIG_DIR as the global Claude folder', () => {
    process.env.CLAUDE_CONFIG_DIR = join(homedir(), 'custom-wormgpt')

    expect(
      isInGlobalClaudeFolder(
        join(homedir(), 'custom-wormgpt', 'settings.json'),
      ),
    ).toBe(false)
  })

  test('global skill scope recognizes ~/.wormgpt and legacy ~/.claude skills', () => {
    process.env.CLAUDE_CONFIG_DIR = join(homedir(), '.wormgpt')

    expect(
      getClaudeSkillScope(
        join(homedir(), '.wormgpt', 'skills', 'demo', 'SKILL.md'),
      ),
    ).toEqual({
      skillName: 'demo',
      pattern: '~/.wormgpt/skills/demo/**',
    })
  ...
  test('global skill scope does not emit fixed rules for arbitrary CLAUDE_CONFIG_DIR skills', () => {
    ...
    expect(
      getClaudeSkillScope(
        join(homedir(), 'custom-wormgpt', 'skills', 'demo', 'SKILL.md'),
      ),
    ).toBe(null)
  })
})

describe('WormGPT validation tips', () => {
...
describe('WormGPT permission mode surfaces', () => {
