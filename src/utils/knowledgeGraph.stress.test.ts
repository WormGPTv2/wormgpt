...
describe('KnowledgeGraph Phase 1 Stress & Edge Cases', () => {
  ...
  const originalOrama = process.env.WORMGPT_KNOWLEDGE_ORAMA
  const configDir = mkdtempSync(join(tmpdir(), 'wormgpt-stress-'))
  ...
  beforeEach(async () => {
    ...
    process.env.WORMGPT_KNOWLEDGE_ORAMA = '1'
  ...
  afterEach(() => {
    try {
      ...
      if (originalOrama === undefined) {
        delete process.env.WORMGPT_KNOWLEDGE_ORAMA
      } else {
        process.env.WORMGPT_KNOWLEDGE_ORAMA = originalOrama
      }
