...
export function getProtocolBinaryName(platform = process.platform): string {
  const baseName =
    MACRO.PACKAGE_URL === '@anthropic-ai/claude-code' ? 'claude' : 'wormgpt'
 ...
 * a cached flag in ~/.wormgpt.json, so:
...
export async function ensureDeepLinkProtocolRegistered(): Promise<void> {
  ...
  // ~/.claude (per-machine, not synced) rather than ~/.wormgpt.json (can sync).
