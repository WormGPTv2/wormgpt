...
describe('KnowledgeGraph Global Persistence & RAG', () => {
  ...
  beforeEach(async () => {
    ...
    configDir = mkdtempSync(join(tmpdir(), 'wormgpt-test-'))
  ...
  it('persists entities across loads', async () => {
    await addGlobalEntity('tool', 'wormgpt', { status: 'alpha' })
    ...
    const entities = Object.values(graph.entities).filter(e => e.name === 'wormgpt')
  ...
  it('deduplicates entities and updates attributes', async () => {
    await addGlobalEntity('tool', 'wormgpt', { status: 'alpha' })
    await addGlobalEntity('tool', 'wormgpt', { status: 'beta', version: '0.6.0' })
