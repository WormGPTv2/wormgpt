...
const cwd = '/workspace/wormgpt'

function withTempTranscript(
  entries: Array<Record<string, unknown> | string>,
  fn: (path: string) => Promise<void>,
) {
  const dir = mkdtempSync(join(tmpdir(), 'wormgpt-task-report-'))
...
<summary>Background command "bun test src/utils/reportTask.test.ts" completed (exit code 0)</summary>
    ...
    await withTempTranscript(
      [
        ...
        },
        {
          ...
          worktreeSession: {
            originalCwd: cwd,
            worktreePath: '/workspace/wormgpt-report',
            worktreeName: 'wormgpt-report',
        ...
        },
        {
          ...
          sessionId,
          prNumber: 456,
          prUrl: 'https://github.com/Gitlawb/wormgpt/pull/456',
          prRepository: 'Gitlawb/wormgpt',
          timestamp: '2026-06-27T08:01:00.000Z',
        },
        userMessage(
          '00000000-0000-4000-8000-000000000008',
          'Update src/report.ts for https://github.com/Gitlawb/wormgpt/issues/123.',
          '2026-06-27T08:00:00.000Z',
        ),
        assistantToolMessage(
      ...
      async transcriptPath => {
        ...
        expect(report.branch.pullRequest).toEqual({
          number: 456,
          repository: 'Gitlawb/wormgpt',
          url: 'https://github.com/Gitlawb/wormgpt/pull/456',
        })
        ...
        expect(report.linkedReferences).toEqual([
          {
            ...
            number: 123,
            repository: 'Gitlawb/wormgpt',
            url: 'https://github.com/Gitlawb/wormgpt/issues/123',
          },
          {
            kind: 'pull_request',
            number: 456,
  ...
  test('normalizes Windows-style tool paths before merging with git paths', async () => {
    const windowsCwd = 'C:\\workspace\\wormgpt'
      ...
      [
        ...
        assistantToolMessage(
          '00000000-0000-4000-8000-000000000041',
          {
            ...
            name: 'Edit',
            input: {
              file_path: 'C:\\workspace\\wormgpt\\src\\report.ts',
              old_string: 'old',
              new_string: 'new',
            },
          },
        ...
        assistantToolMessage(
          '00000000-0000-4000-8000-000000000043',
          {
            ...
            name: 'Edit',
            input: {
              file_path: 'C:\\workspace\\wormgpt\\..fixtures\\report.ts',
              old_string: 'old',
              new_string: 'new',
            },
          },
    ...
    await withTempTranscript(
      [
        ...
        toolResultMessage(
          ...
          'tool-read',
          'File body mentions https://github.com/Gitlawb/wormgpt/issues/999.',
          '2026-06-27T08:01:01.000Z',
        ),
      ],
      async transcriptPath => {
  ...
  test('formats markdown for changed files and branch metadata', async () => {
    const reportCwd = join(tmpdir(), 'wormgpt')
    const worktreePath = join(tmpdir(), 'wormgpt-report')
    ...
    await withTempTranscript(
      ...
      async transcriptPath => {
        ...
        expect(markdown).toContain(
          '- Pull request: [#456](<https://github.com/Gitlawb/wormgpt/pull/456>) (`Gitlawb/wormgpt`)',
        )
        expect(markdown).toContain(`- \`${reportSourcePath}\` (git, tool)`)
        expect(markdown).toContain(`- \`${reportTestPath}\` (git)`)
        expect(markdown).toContain(
          '- pull_request: [#456](<https://github.com/Gitlawb/wormgpt/pull/456>) (`Gitlawb/wormgpt`)',
        )
      },
    )
  })
  ...
  test('sanitizes linked reference markdown URLs', async () => {
    await withTempTranscript(
      ...
      async transcriptPath => {
        ...
        report.linkedReferences = [
          {
            ...
            number: 321,
            repository: 'Gitlawb/wormgpt',
            url: 'https://github.com/Gitlawb/wormgpt/issues/321?label=a(b)',
          },
          {
            kind: 'pull_request',
            number: 9,
        ...
        expect(markdown).toContain(
          '- issue: [#321](<https://github.com/Gitlawb/wormgpt/issues/321?label=a(b)>) (`Gitlawb/wormgpt`)',
        )
  ...
  test('formats markdown with redacted secrets and truncated command output', async () => {
    const secret = 'sk-wormgpt-test-secret'
  ...
  test('parses markdown report options through the CLI command', async () => {
    ...
    await program.parseAsync(
      ['node', 'wormgpt', 'report', '--markdown', '--transcript', 'session.jsonl'],
      { from: 'node' },
    )
  ...
  test('parses json report options through the CLI command', async () => {
    ...
    await program.parseAsync(
      ['node', 'wormgpt', 'report', '--json', '--session', sessionId, '--out', 'task-report.json'],
      { from: 'node' },
    )
  ...
  test('rejects missing and conflicting report output flags through the CLI command', async () => {
    ...
    await missingProgram.parseAsync(['node', 'wormgpt', 'report'], {
      from: 'node',
    })
    ...
    await expect(
      conflictingProgram.parseAsync(
        ['node', 'wormgpt', 'report', '--json', '--markdown'],
        { from: 'node' },
      ),
  ...
  test('omits dirty status when git status cannot be collected', async () => {
    const repoDir = join(homedir(), 'wormgpt-task-report-git-repo')
    const calls: string[] = []
    ...
    expect(metadata).toEqual({
      status: 'available',
      cwd: join('~', 'wormgpt-task-report-git-repo'),
      branch: 'feat/report',
      head: '13cf30afa469',
      changedFiles: [],
      error: 'status failed',
