...
const PROJECT_CONFIG_DIR_NAMES = ['.claude', '.wormgpt'] as const
...
// ~/.wormgpt/agents — see issue #769) opens that many fds and blocks the
...
function recordOversizedSkip(skip: OversizedMarkdownSkip): void {
  ...
  if (!oversizedSkipStderrWarned) {
    ...
    process.stderr.write(
      `wormgpt: skipping oversized markdown config file ${skip.filePath} ` +
