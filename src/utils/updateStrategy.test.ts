...
describe('isThirdPartyBuildBlockedFor', () => {
  ...
  test('allows a custom-PACKAGE_URL build (WormGPT) on any provider', () => {
    expect(isThirdPartyBuildBlockedFor('bedrock', WORMGPT)).toBe(false)
    expect(isThirdPartyBuildBlockedFor('firstParty', WORMGPT)).toBe(false)
  })
})
