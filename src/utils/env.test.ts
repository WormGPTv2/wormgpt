...
beforeEach(async () => {
  ...
  tempDir = mkdtempSync(join(tmpdir(), 'wormgpt-env-test-'))
...
afterEach(() => {
  try {
    ...
    if (originalEnv.WORMGPT_CONFIG_DIR === undefined) {
      delete process.env.WORMGPT_CONFIG_DIR
    } else {
      process.env.WORMGPT_CONFIG_DIR = originalEnv.WORMGPT_CONFIG_DIR
    }
...
test('getGlobalClaudeFile: new install returns .wormgpt.json when neither file exists', async () => {
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.wormgpt.json'))
})
...
test('getGlobalClaudeFile: migrated user uses .wormgpt.json when both files exist', async () => {
  writeFileSync(join(tempDir, '.claude.json'), '{}')
  writeFileSync(join(tempDir, '.wormgpt.json'), '{}')
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.wormgpt.json'))
})

test('getGlobalClaudeFile: WORMGPT_CONFIG_DIR uses preferred config dir', async () => {
  const preferredDir = mkdtempSync(join(tmpdir(), 'wormgpt-preferred-env-test-'))
  try {
    process.env.WORMGPT_CONFIG_DIR = preferredDir
    process.env.CLAUDE_CONFIG_DIR = tempDir

    const { getGlobalClaudeFile } = await importFreshEnvModule()

    expect(getGlobalClaudeFile()).toBe(join(preferredDir, '.wormgpt.json'))
  } finally {
    rmSync(preferredDir, { recursive: true, force: true })
  }
})

test('getGlobalClaudeFile: WORMGPT_CONFIG_DIR keeps .claude.json fallback when only legacy file exists', async () => {
  const preferredDir = mkdtempSync(join(tmpdir(), 'wormgpt-preferred-env-test-'))
  try {
    process.env.WORMGPT_CONFIG_DIR = preferredDir
    process.env.CLAUDE_CONFIG_DIR = tempDir
