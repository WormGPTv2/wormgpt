...
export async function detectStaleProjectSettingsPaths(
  ...
  return {
    issue: `Legacy project settings file${stale.length === 1 ? '' : 's'} ${legacyPaths} found, but WormGPT reads ${canonicalPaths}`,
    fix: `Move or copy ${legacyPaths} to ${canonicalPaths} if you intended WormGPT to use those project settings.`,
  }
}
    ...
    if (!claudeInPath && !validAlias) {
      if (existingAlias) {
        ...
        warnings.push({
          ...
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias ${getCliBinaryName()}="~/.wormgpt/local/${getCliBinaryName()}"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Create alias: alias ${getCliBinaryName()}="~/.wormgpt/local/${getCliBinaryName()}"`,
        })
      }
    }
  }
