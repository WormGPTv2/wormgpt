...
describe('getGlobalInstallArgs', () => {
  const spec = '@gitlawb/wormgpt@latest'
  ...
  test('passes an explicit version spec through unchanged', () => {
    expect(getGlobalInstallArgs('npm', '@gitlawb/wormgpt@1.2.3')).toEqual([
      'install',
      '-g',
      '@gitlawb/wormgpt@1.2.3',
    ])
  })
})
...
describe('selectOwningPackageManager', () => {
  ...
  test('matches the package manager whose root contains the binary', () => {
    expect(
      selectOwningPackageManager(
        '/home/u/.local/share/pnpm/global/5/node_modules/@gitlawb/wormgpt/cli.js',
        [
          { pm: 'npm', root: '/usr/local/lib/node_modules' },
          {
            pm: 'pnpm',
  ...
  test('most specific (longest) root wins when roots are nested', () => {
    ...
    expect(
      selectOwningPackageManager('/opt/pm/node_modules/bun/global/wormgpt', [
  ...
  test('does not match a sibling directory sharing a prefix', () => {
    ...
    expect(
      selectOwningPackageManager('/a/node_modules-other/wormgpt/cli.js', [
  ...
  test('ignores candidates with an empty root', () => {
    expect(
      selectOwningPackageManager('/usr/lib/node_modules/wormgpt', [
        { pm: 'yarn', root: '' },
