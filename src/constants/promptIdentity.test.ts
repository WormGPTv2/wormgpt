...
beforeAll(async () => {
  ...
  ;(globalThis as Record<string, unknown>).MACRO = {
    ...
    ISSUES_EXPLAINER:
      'report the issue at https://github.com/Gitlawb/wormgpt/issues',
...
test('CLI identity prefixes describe WormGPT instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('WormGPT')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('WormGPT')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes WormGPT instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('WormGPT')
...
test('built-in agent prompts describe WormGPT instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('WormGPT')
  ...
  expect(generalPrompt).toContain('WormGPT')
  ...
  expect(explorePrompt).toContain('WormGPT')
  ...
  expect(guidePrompt).toContain('WormGPT')
  expect(guidePrompt).toContain('You are the WormGPT guide agent.')
  expect(guidePrompt).toContain('**WormGPT** (the CLI tool)')
