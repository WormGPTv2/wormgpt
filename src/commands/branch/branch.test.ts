...
async function setupSourceTranscript(
  ...
  const projectCwd = await mkdtemp(join(tmpdir(), 'wormgpt-branch-cwd-'))
  const configDir = await mkdtemp(join(tmpdir(), 'wormgpt-branch-config-'))
  const sessionProjectDir = options.separateSessionProjectDir
    ? await mkdtemp(join(tmpdir(), 'wormgpt-branch-session-dir-'))
    : null
