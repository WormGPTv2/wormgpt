...
describe('background session registry', () => {
  ...
  beforeEach(async () => {
    configDir = await mkdtemp(join(tmpdir(), 'wormgpt-bg-registry-'))
  ...
  it('creates session metadata and log files under the WormGPT config dir', async () => {
    const session = await createBackgroundSession({
      id: 'bg-test-1',
      name: 'auth-refactor',
      pid: 12345,
      cwd: '/repo',
      command: ['wormgpt', '--print', 'refactor auth'],
      provider: 'openai',
      model: 'gpt-5',
  ...
  it('does not steal an in-flight name reservation from a live creator', async () => {
    ...
    await expect(
      createBackgroundSession({
        ...
        pid: 222,
        cwd: '/repo',
        command: ['wormgpt', '--print', 'contender'],
  ...
  it('recovers orphaned name reservations whose owner metadata is missing', async () => {
    ...
    const session = await createBackgroundSession({
      ...
      pid: 222,
      cwd: '/repo',
      command: ['wormgpt', '--print', 'recovered'],
  ...
  it('rejects non-positive pids at creation', async () => {
    ...
    await expect(
      createBackgroundSession({
        ...
        pid: -1,
        cwd: '/repo',
        command: ['wormgpt', '--print', 'negative'],
  ...
  it('keeps running sessions fresh when their process identity still matches', async () => {
    await createBackgroundSession({
      ...
      pid: 333,
      cwd: '/repo',
      command: ['wormgpt', '--session-id', 'conversation-1', '--print', 'work'],
      sessionId: 'conversation-1',
      now: new Date('2026-06-15T08:00:00.000Z'),
    })
    ...
    const refreshed = await refreshBackgroundSessionStatuses({
      isProcessAlive: () => true,
      getProcessCommand: () =>
        'node wormgpt --session-id conversation-1 --print work',
      now: new Date('2026-06-15T08:05:00.000Z'),
    })
  ...
  it('keeps PR-resume sessions fresh when the live command matches the stored invocation', async () => {
    await createBackgroundSession({
      ...
      pid: 333,
      cwd: '/repo',
      command: ['wormgpt', '--from-pr', '1642', '--print'],
    ...
    const refreshed = await refreshBackgroundSessionStatuses({
      isProcessAlive: () => true,
      getProcessCommand: () => 'node wormgpt --from-pr 1642 --print',
      now: new Date('2026-06-15T08:05:00.000Z'),
    })
