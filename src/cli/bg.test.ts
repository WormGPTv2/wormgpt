...
async function withTempFile<T>(
  ...
  const dir = await mkdtemp(join(tmpdir(), 'wormgpt-bg-test-'))
  try {
...
describe('background session CLI parsing', () => {
  ...
  it('preserves Node exec flags and lets the launcher manage heap relaunch state', () => {
    const config = buildBackgroundChildProcessConfig({
      ...
      entrypoint: '/repo/bin/wormgpt',
      childArgs: ['--print', 'fix failing tests'],
      processEnv: {
        WORMGPT_HEAP_RELAUNCHED: '1',
        WORMGPT_NODE_MAX_OLD_SPACE_SIZE_MB: '8192',
      },
      sessionName: 'tests',
    ...
    expect(config.env.WORMGPT_HEAP_RELAUNCHED).toBeUndefined()
    expect(config.env.WORMGPT_NODE_MAX_OLD_SPACE_SIZE_MB).toBe('8192')
