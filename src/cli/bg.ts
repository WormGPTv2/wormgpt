...
const HEAP_RELAUNCHED_ENV = 'WORMGPT_HEAP_RELAUNCHED'
...
export async function logsHandler(
  ...
  if (!parsed.target) fail('Usage: wormgpt logs <id-or-name> [-f]')
...
export async function attachHandler(
  ...
  if (!target) fail('Usage: wormgpt attach <id-or-name>')

  await refreshBackgroundSessionStatuses()
  const session = await resolveSessionOrExit(target)
  console.error(
    `Attach is not implemented for local background sessions yet. Use \`wormgpt logs ${session.id} -f\` to follow output.`,
  )
  process.exitCode = 1
}
...
export async function killHandler(
  ...
  const target = normalizeArgs(args)[0]
  if (!target) fail('Usage: wormgpt kill <id-or-name>')
...
export async function handleBgFlag(args: string[]): Promise<void> {
  ...
  if (!parsed.prompt && !hasResumeSource(parsed.childArgs)) {
    fail('Usage: wormgpt --bg [--name <name>] "<prompt>"')
  }
  ...
  if (!entrypoint) {
    fail('Cannot determine WormGPT entrypoint for background session')
  }
  ...
  console.log(`Follow: wormgpt logs ${session.id} -f`)
  console.log(
