...
export function REPL({
  ...
  const terminalTitle = sessionTitle ?? agentTitle ?? haikuTitle ?? 'WormGPT';
  ...
  useEffect(() => {
    ...
    setMessages(prev => [...prev, createSystemMessage(`Worktree creation took ${secs}s. For large repos, set \`worktree.sparsePaths\` in .wormgpt/settings.json to check out only the directories you need — e.g. \`{"worktree": {"sparsePaths": ["src", "packages/foo"]}}\`.`, 'info')]);
  }, [setMessages]);
  ...
  // That write storm is the primary trigger for ~/.wormgpt.json corruption
  // (GH #3117).
  ...
  useEffect(() => {
    const handleSuspend = () => {
      ...
      process.stdout.write(`\nWormGPT has been suspended. Run \`fg\` to bring WormGPT back.\nNote: ctrl + z now suspends WormGPT, ctrl + _ undoes input.\n`);
    };
    const handleResume = () => {
