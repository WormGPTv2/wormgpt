 ...
 * Closes: Gitlawb/wormgpt#402 — JavaScript heap OOM during large tasks
 */
...
describe('cli.tsx — --provider startup ordering', () => {
  ...
  beforeEach(() => {
    clearRememberedProviderFlagForTests()
    tempDir = mkdtempSync(join(tmpdir(), 'wormgpt-cli-env-file-test-'))
...
describe('cli.tsx — background routing behavior', () => {
  ...
  const originalAutoRunGuard =
    process.env.WORMGPT_DISABLE_CLI_ENTRYPOINT_AUTO_RUN

  beforeAll(async () => {
    process.env.WORMGPT_DISABLE_CLI_ENTRYPOINT_AUTO_RUN = '1'
  ...
  afterAll(() => {
    if (originalAutoRunGuard === undefined) {
      delete process.env.WORMGPT_DISABLE_CLI_ENTRYPOINT_AUTO_RUN
    } else {
      process.env.WORMGPT_DISABLE_CLI_ENTRYPOINT_AUTO_RUN =
