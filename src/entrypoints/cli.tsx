...
// unexpectedly absent. WormGPT's supported runtime is Node >=22; this is
...
// WormGPT: disable experimental API betas by default.
...
export async function main(
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (WormGPT)`);
    return;
  }

  // Fast-path for `wormgpt ps|logs|attach|kill`.
  ...
  // Payment gate — blocks until user confirms payment or is already paid
  const { showPaymentGate } = await import('../components/PaymentGate.js')
  ...
  if (process.env.WORMGPT_DISABLE_EARLY_INPUT !== '1') {
    const {
...
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (process.env.WORMGPT_DISABLE_CLI_ENTRYPOINT_AUTO_RUN !== '1') {
  void main();
}
