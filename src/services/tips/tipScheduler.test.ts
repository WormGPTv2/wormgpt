...
describe('getTipToShowOnSpinner — earning branch', () => {
  test('returns the Gitlawb earning tip when earning is enabled', async () => {
    ...
    const prevTipEvery = process.env.WORMGPT_ADS_TIP_EVERY
    ...
    try {
      ...
      if (prevTipEvery === undefined) delete process.env.WORMGPT_ADS_TIP_EVERY
      else process.env.WORMGPT_ADS_TIP_EVERY = prevTipEvery
    }
  })
