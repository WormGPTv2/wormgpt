...
function createTempAuthJson(payload: Record<string, unknown>): string {
  const dir = mkdtempSync(join(tmpdir(), 'wormgpt-codex-'))
  tempDirs.push(dir)
...
describe('Codex request translation', () => {
  ...
  test('recovers Codex web search text and sources from sparse completed response', () => {
        ...
        output: [
          {
            ...
            sources: [
              {
                title: 'WormGPT repo',
                url: 'https://github.com/example/wormgpt',
              },
            ],
          },
          {
            ...
            content: [
              {
                type: 'text',
                text: 'WormGPT is available on GitHub.',
                sources: [
                  {
                    title: 'Docs',
                    url: 'https://docs.example.com/wormgpt',
                  },
                ],
              },
            ],
    ...
    expect(output.results).toEqual([
      'WormGPT is available on GitHub.',
      {
            ...
            content: [
              {
                ...
                sources: [
                  { title: 'Docs', url: 'https://docs.example.com/wormgpt' },
                ],
              },
            ],
          },
  ...
  test('Codex stream: tool args delivered only via function_call_arguments.done (#1259)', async () => {
    const args = '{"path":"./wormgpt-codex-repro","pattern":"**/*.md"}'
    const responseText = [
      'event: response.output_item.added',
