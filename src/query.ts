...
function createAutoCompactDiagnosticMessage(args: {
  ...
  if (circuitBreakerActive || circuitBreakerTripped) {
    ...
    return createSystemMessage(
      `Automatic compaction is paused after repeated failures.${retryText} WormGPT will stop before sending oversized requests while the guard is active.`,
      'warning',
    )
  }
  ...
  if (
    ...
    return createSystemMessage(
      `Automatic compaction failed (${consecutiveFailures}/${MAX_CONSECUTIVE_AUTOCOMPACT_FAILURES}); WormGPT will retry compaction on the next eligible turn.`,
      'warning',
    )
  }
...
function createContextOverflowRecoveryMessage(): UserMessage {
  return createUserMessage({
    content:
      'The previous provider request exceeded the context window. WormGPT compacted the conversation and is retrying this turn once; continue from the compacted context, avoid repeating the oversized request shape, and use narrower tool reads if more detail is needed.',
    isMeta: true,
  })
}
  ...
  while (true) {
    ...
    const activeMessageLimit = canForceCompact
      ? resolveMaxActiveMessagesLimit(
          ...
          process.env.WORMGPT_MAX_ACTIVE_MESSAGES,
        )
      : 0
    if (canForceCompact) {
      if (
        ...
        const content =
          retryDelayMs !== undefined && retryDelayMs > 0
            ? 'The conversation is over the auto-compact safety threshold, but automatic compaction is cooling down after repeated failures. ' +
              'WormGPT stopped before sending another oversized request. ' +
            ...
            : 'The conversation is over the auto-compact safety threshold and automatic compaction has failed repeatedly. ' +
              'WormGPT stopped before sending another oversized request. Run /compact, undo recent large tool output, or start a new session with /new.'
        yield createAssistantAPIErrorMessage({
          content,
          error: 'invalid_request',
        })
    ...
    if (
      ...
      yield createAssistantAPIErrorMessage({
        content:
          'The conversation is over the active-message safety limit, but automatic compaction could not reduce it before the next provider request. WormGPT stopped before sending another oversized request. Run /compact, undo recent large tool output, or start a new session with /new.',
        error: 'invalid_request',
      })
      return { reason: 'blocking_limit' }
    }
