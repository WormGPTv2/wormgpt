...
describe('App stdin mode setup', () => {
  const originalDataMode = process.env.WORMGPT_USE_DATA_STDIN
  const originalReadableMode = process.env.WORMGPT_USE_READABLE_STDIN

  afterEach(() => {
    if (originalDataMode === undefined) {
      delete process.env.WORMGPT_USE_DATA_STDIN
    } else {
      process.env.WORMGPT_USE_DATA_STDIN = originalDataMode
    }
    if (originalReadableMode === undefined) {
      delete process.env.WORMGPT_USE_READABLE_STDIN
    } else {
      process.env.WORMGPT_USE_READABLE_STDIN = originalReadableMode
    }
  })
  ...
  test('resumes stdin only for opt-in data mode', () => {
    process.env.WORMGPT_USE_DATA_STDIN = '1'
    delete process.env.WORMGPT_USE_READABLE_STDIN
    const stdin = createFakeStdin()
    const app = createApp(stdin)
  ...
  test('uses data mode when WORMGPT_USE_READABLE_STDIN=0', () => {
    delete process.env.WORMGPT_USE_DATA_STDIN
    process.env.WORMGPT_USE_READABLE_STDIN = '0'
    const stdin = createFakeStdin()
    const app = createApp(stdin)
