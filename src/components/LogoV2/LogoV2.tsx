...
import { BRAND_TAGLINE, WORDMARK_GPT, WORDMARK_WORM } from '../../constants/brand.js';
...
export function LogoV2() {
  ...
  const borderTitle = ` ${color("text", userTheme)("WormGPT")} ${color("inactive", userTheme)(`v${version}`)} `;
  const compactBorderTitle = color("text", userTheme)(" WormGPT ");
  ...
  if ($[46] !== welcomeMessage_0) {
    t18 = <Box marginTop={1} flexDirection="column" alignItems="center"><Text><Text color="brandShimmer">{WORDMARK_WORM[0]}</Text><Text color="brand"> {WORDMARK_GPT[0]}</Text></Text><Text><Text color="brandShimmer">{WORDMARK_WORM[1]}</Text><Text color="brand"> {WORDMARK_GPT[1]}</Text></Text><Text dimColor={true}>{BRAND_TAGLINE}</Text><Text color="inactive">•</Text><Text bold={true}>{welcomeMessage_0}</Text></Box>;
    $[46] = welcomeMessage_0;
    $[47] = t18;
  } else {
    t18 = $[47];
