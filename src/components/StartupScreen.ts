/**
 * WormGPT startup screen — filled-block text logo with sunset gradient.
 * Called once at CLI startup before the Ink UI renders.
 *
 * Addresses: https://github.com/Gitlawb/wormgpt/issues/55
 */
...
const LOGO_WORMGPT = [
  `  __          ______  _____  __  __  _____ _____ _______ `,
  `  \\\\ \\        / / __ \\\\|  __ \\\\|  \\\\/  |/ ____|  __ \\\\__   __|`,
  `   \\\\ \\\\  /\\\\  / / |  | | |__) | \\\\  / | |  __| |__) | | |   `,
  `    \\\\ \\\\/  \\\\/ /| |  | |  _  /| |\\\\/| | | |_ |  ___/  | |   `,
  `     \\\\  /\\\\  / | |__| | | \\\\ \\\\| |  | | |__| | |      | |   `,
...
export function printStartupScreen(modelOverride?: string): void {
  ...
  const p = { name: 'Anthropic', model: 'claude-fable-5', baseUrl: 'https://api.anthropic.com/v1/messages', isLocal: false }
  const W = 62
  const out: string[] = []
  ...
  out.push(`${ansiRgb(...BORDER)}\u255a${'\u2550'.repeat(W - 2)}\u255d${RESET}`)
  out.push(`  ${DIM}${ansiRgb(...DIMCOL)}wormgpt ${RESET}${ansiRgb(...ACCENT)}v${MACRO.DISPLAY_VERSION ?? MACRO.VERSION}${RESET}`)

  // Welcome
  out.push('')
  out.push(`  ${ansiRgb(...ACCENT)}\u25c6${RESET} ${ansiRgb(...CREAM)}Welcome to ${ansiRgb(...ACCENT)}WormGPT${RESET} ${ansiRgb(...ACCENT)}\u25c6${RESET}`)
  out.push('')
  out.push('')
