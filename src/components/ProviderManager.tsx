...
function XaiOAuthSetup({
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Text>
        Sign in with your xAI account in the browser. WormGPT will store
      ...
      <Text dimColor>
        ...
        expected. WormGPT uses xAI's shared OAuth client.
      </Text>
...
function CodexOAuthSetup({
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Text>
        Sign in with your ChatGPT account in the browser. WormGPT will store
...
export function ProviderManager({ mode, onDone }: Props): React.ReactNode {
  ...
  function buildCodexOAuthActivationMessage(options: {
    ...
    if (options.warnings.length > 0) {
      return `${options.prefix}. WormGPT switched to it for this session with warnings: ${formatWarningsForMessage(options.warnings)}`
    }

    return `${options.prefix}. WormGPT switched to it for this session.`
  }
  ...
  function buildXaiOAuthActivationMessage(options: {
    ...
    if (options.warnings.length > 0) {
      return `${options.prefix}. WormGPT switched to it for this session with warnings: ${options.warnings.join('; ')}.`
    }
    return `${options.prefix}. WormGPT switched to it for this session.`
  }
