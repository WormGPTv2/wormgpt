...
export const settingsFiles: SettingsFile[] = [
  {
    path: '~/.wormgpt/settings.json',
    scope: 'user',
    notes: 'Default global settings path for every project on the machine; WORMGPT_CONFIG_DIR moves this under the configured config home.',
  },
  {
    path: '.wormgpt/settings.json',
    scope: 'project',
  ...
  {
    path: '.wormgpt/settings.local.json',
    scope: 'local',
    notes: 'Per-machine overrides for one project; typically gitignored.',
  },
  {
    path: '~/.wormgpt/keybindings.json',
    scope: 'user',
    notes: 'Default keyboard shortcut overrides path; WORMGPT_CONFIG_DIR moves this under the configured config home.',
  },
  {
...
export const envVars: EnvVar[] = [
  ...
  { name: 'WORMGPT_CONFIG_DIR', description: 'Preferred config directory override. Defaults to ~/.wormgpt when unset.' },
  { name: 'CLAUDE_CONFIG_DIR', description: 'Legacy config directory override. Used only when WORMGPT_CONFIG_DIR is unset.' },
