# WormGPT Local Agent Playbook

This playbook is a practical guide to run WormGPT with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, WormGPT starts directly.
...
Or let WormGPT recommend the best local model for your goal:
...
```powershell
Get-Content .\.wormgpt-profile.json
```
...
- Keep any legacy workspace `.wormgpt-profile.json` local; new provider saves use the user config directory.
