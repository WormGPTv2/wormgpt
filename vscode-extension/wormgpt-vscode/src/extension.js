...
const { ChatController, WormGPTChatViewProvider, WormGPTChatPanelManager } = require('./chat/chatProvider');
const { SessionManager } = require('./chat/sessionManager');
const { DiffContentProvider, SCHEME: DIFF_SCHEME } = require('./chat/diffController');

const WORMGPT_REPO_URL = 'https://github.com/Gitlawb/wormgpt';
const WORMGPT_SETUP_URL = 'https://github.com/Gitlawb/wormgpt/blob/main/README.md#quick-start';
const PROFILE_FILE_NAME = '.wormgpt-profile.json';
const SECRET_AZURE_API_KEY = 'wormgpt.azure.apiKey';
...
async function buildLaunchAzureEnv(configured) {
  ...
  if (!endpoint || !deployment) {
    void vscode.window.showWarningMessage(
      'WormGPT Azure chat is enabled but endpoint or deployment is missing. Run "WormGPT: Configure Azure / Foundry Chat" or set wormgpt.azure.* in settings.',
    );
    return env;
  }
  if (!apiKey) {
    void vscode.window.showWarningMessage(
      'WormGPT Azure chat is enabled but no API key is set. Use "WormGPT: Set Azure / Foundry API Key" or wormgpt.azure.apiKey (not recommended).',
    );
    return env;
  }
...
async function setAzureApiKey(context) {
  const key = await vscode.window.showInputBox({
    title: 'WormGPT — Azure / Foundry API key',
  ...
  void vscode.window.showInformationMessage('WormGPT Azure / Foundry API key saved to Secret Storage.');
}
...
async function clearAzureApiKey(context) {
  ...
  void vscode.window.showInformationMessage('WormGPT Azure / Foundry API key removed from Secret Storage.');
}
...
async function configureAzureChat(context) {
  const cfg = vscode.workspace.getConfiguration('wormgpt');
  const target = vscode.ConfigurationTarget.Global;

  const endpoint = await vscode.window.showInputBox({
    title: 'WormGPT — Azure / Foundry API endpoint',
  ...
  const apiVersion = await vscode.window.showInputBox({
    title: 'WormGPT — Azure API version',
  ...
  const deployment = await vscode.window.showInputBox({
    title: 'WormGPT — Azure deployment / model',
  ...
  void vscode.window.showInformationMessage(
    'WormGPT Azure / Foundry chat saved. Launch WormGPT from the Control Center or command palette.',
  );
}
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('wormgpt');
  const launchCommand = configured.get('launchCommand', 'wormgpt');
  const terminalName = configured.get('terminalName', 'WormGPT');
...
async function launchWormGPT(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `WormGPT command not found: ${executable}. Install it with: npm install -g @gitlawb/wormgpt@latest`,
      'Open Setup Guide',
      'Open Repository',
    );

    if (action === 'Open Setup Guide') {
      await vscode.env.openExternal(vscode.Uri.parse(WORMGPT_SETUP_URL));
    } else if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(WORMGPT_REPO_URL));
    }
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch WormGPT because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="WormGPT wordmark">Open<span class="wordmark-accent">Claude</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the upstream WormGPT project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and WormGPT commands quickly.</span>
            </button>
...
class WormGPTControlCenterProvider {
  constructor() {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          ...
          break;
        case 'launchRoot':
          await launchWormGPT({ requireWorkspace: true });
          break;
        case 'openProfile':
        ...
        case 'commands':
          ...
          break;
        case 'azureSettings':
          await vscode.commands.executeCommand('workbench.action.openSettings', 'wormgpt.azure');
          break;
        case 'refresh':
        default:
          break;
...
function activate(context) {
  ...
  const provider = new WormGPTControlCenterProvider();
  ...
  const chatViewProvider = new WormGPTChatViewProvider(chatController);
  const chatPanelManager = new WormGPTChatPanelManager(chatController);
  ...
  statusBarItem.text = '$(comment-discussion) WormGPT';
  statusBarItem.tooltip = 'Open WormGPT Chat';
  statusBarItem.command = 'wormgpt.openChat';
  statusBarItem.show();

  chatController.onDidChangeState((state) => {
    switch (state) {
      case 'streaming':
        statusBarItem.text = '$(sync~spin) WormGPT';
        statusBarItem.tooltip = 'WormGPT is generating...';
        break;
      case 'connected':
        statusBarItem.text = '$(comment-discussion) WormGPT';
        statusBarItem.tooltip = 'WormGPT connected';
        break;
      default:
        statusBarItem.text = '$(comment-discussion) WormGPT';
        statusBarItem.tooltip = 'Open WormGPT Chat';
  ...
  // ── Existing commands ──
  const startCommand = vscode.commands.registerCommand('wormgpt.start', async () => {
    await launchWormGPT();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'wormgpt.startInWorkspaceRoot',
    async () => {
      await launchWormGPT({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('wormgpt.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(WORMGPT_REPO_URL));
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'wormgpt.openSetupDocs',
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse(WORMGPT_SETUP_URL));
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'wormgpt.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('wormgpt.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.wormgpt');
  });

  // ── New chat commands ──
  const newChatCommand = vscode.commands.registerCommand('wormgpt.newChat', () => {
    chatController.stopSession();
    chatController.broadcast({ type: 'session_cleared' });
  });

  const openChatCommand = vscode.commands.registerCommand('wormgpt.openChat', () => {
    chatPanelManager.openPanel();
  });

  const resumeSessionCommand = vscode.commands.registerCommand('wormgpt.resumeSession', async () => {
  ...
  const abortChatCommand = vscode.commands.registerCommand('wormgpt.abortChat', () => {
    chatController.abort();
  });

  const setAzureApiKeyCommand = vscode.commands.registerCommand('wormgpt.setAzureApiKey', async () => {
    await setAzureApiKey(context);
  });

  const clearAzureApiKeyCommand = vscode.commands.registerCommand('wormgpt.clearAzureApiKey', async () => {
    await clearAzureApiKey(context);
  });

  const configureAzureChatCommand = vscode.commands.registerCommand('wormgpt.configureAzureChat', async () => {
    await configureAzureChat(context);
  });

  const openAzureSettingsCommand = vscode.commands.registerCommand('wormgpt.openAzureSettings', async () => {
    await vscode.commands.executeCommand('workbench.action.openSettings', 'wormgpt.azure');
  });

  // ── Register providers ──
  const controlCenterProviderReg = vscode.window.registerWebviewViewProvider(
    'wormgpt.controlCenter',
    provider,
  );
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('wormgpt')) {
        refreshProvider();
      }
    }),
...
module.exports = {
  activate,
  deactivate,
  WormGPTControlCenterProvider,
  renderControlCenterHtml,
  resolveLaunchTargets,
  ChatController,
  WormGPTChatViewProvider,
  WormGPTChatPanelManager,
};
