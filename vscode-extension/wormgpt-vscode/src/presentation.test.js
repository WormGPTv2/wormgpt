...
test('truncateMiddle keeps the profile filename visible', () => {
  ...
  assert.equal(
    truncateMiddle('/Users/example/projects/wormgpt/workspace/.wormgpt-profile.json', 30),
    '.../.wormgpt-profile.json',
  );
});
...
test('truncateMiddle keeps the filename visible for Windows-style paths', () => {
  ...
  assert.equal(
    truncateMiddle('C:\\Users\\example\\wormgpt\\workspace\\.wormgpt-profile.json', 30),
    '...\\.wormgpt-profile.json',
  );
});
...
test('buildActionModel includes workspace-profile action when a profile exists', () => {
  ...
  const model = buildActionModel({
    canLaunchInWorkspaceRoot: true,
    workspaceProfilePath: 'C:\\Users\\example\\wormgpt\\workspace\\.wormgpt-profile.json',
  });

  assert.deepEqual(model.openProfile, {
    id: 'openProfile',
    label: 'Open Workspace Profile',
    detail: 'Inspect ...\\.wormgpt-profile.json',
    tone: 'neutral',
    disabled: false,
  });
});
...
function createStatus(overrides = {}) {
  return {
    ...
    shimEnabled: false,
    workspaceFolder: '/workspace/wormgpt',
    ...
    profileStatusHint: '/workspace/wormgpt/.wormgpt-profile.json',
    workspaceProfilePath: '/workspace/wormgpt/.wormgpt-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
      ...
      rows: [
        {
          ...
          summary: 'wormgpt',
          detail: '/workspace/wormgpt · active editor workspace',
        },
        {
          key: 'profileStatus',
          label: 'Workspace profile',
          summary: 'Found',
          detail: '/workspace/wormgpt/.wormgpt-profile.json',
          tone: 'neutral',
        },
      ],
    },
...
test('buildControlCenterViewModel keeps launch command only in summary cards', () => {
  ...
  assert.deepEqual(viewModel.summaryCards.find(card => card.key === 'launchCommand'), {
    ...
    value: 'wormgpt --project-aware',
    detail: 'Integrated terminal: WormGPT',
  });
