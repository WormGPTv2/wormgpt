...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'wormgpt',
    launchCommand: 'wormgpt --project-aware',
    terminalName: 'WormGPT',
    shimEnabled: false,
    workspaceFolder: '/workspace/wormgpt/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/wormgpt/very/long/path/example-project',
    launchCwdLabel: '/workspace/wormgpt/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/wormgpt/very/long/path/example-project/.wormgpt-profile.json',
    workspaceProfilePath: '/workspace/wormgpt/very/long/path/example-project/.wormgpt-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the WormGPT wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(
    html,
    /title="\/workspace\/wormgpt\/very\/long\/path\/example-project"[^>]*>\/workspace\/wormgpt\/very\/long\/path\/example-project<\//,
  );
});
...
test('WormGPTControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { WormGPTControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/wormgpt/src/panels/control-center.js',
      workspacePath: '/workspace/wormgpt',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/wormgpt/src/panels',
      projectAwareCwdLabel: '/workspace/wormgpt/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/wormgpt',
      workspaceRootCwdLabel: '/workspace/wormgpt',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/wormgpt',
      activeFilePath: '/workspace/wormgpt/src/panels/control-center.js',
      workspacePath: '/workspace/wormgpt',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/wormgpt',
      projectAwareCwdLabel: '/workspace/wormgpt',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/wormgpt/src/panels',
      launchCwdLabel: '/workspace/wormgpt/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/wormgpt\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/wormgpt/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/wormgpt',
      launchCwdLabel: '/workspace/wormgpt',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/wormgpt/);
  assert.match(html, /Same workspace-root target as Launch WormGPT because the relative command resolves from the workspace root · \/workspace\/wormgpt/);
});
